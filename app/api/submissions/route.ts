...
type Answer = { rows?: { column: string; value: string; reason: string }[]; result: string; explanation: string; image: string; imageName: string; imagePath?: string; imageReceipt?: string };
function validAnswer(value: unknown, owner: string): value is Answer {
  ...
  return ['result', 'explanation', 'image', 'imageName'].every(k => typeof a[k as keyof Answer] === 'string')
    ...
    && (a.imagePath === undefined ? (!a.image || /^data:image\/(png|jpeg|webp);base64,[A-Za-z0-9+/]+={0,2}$/.test(a.image) && Buffer.from(a.image.split(',')[1], 'base64').length <= 2 * 1024 * 1024) : !a.image && validImageReceipt(a.imagePath, a.imageReceipt, owner));
}
export async function POST(request: Request) {
  ...
  try {
    ...
    const response = await fetch(`${url.replace(/\/$/, '')}/rest/v1/submissions`, {
      ...
      body: JSON.stringify({ student_name: data.name.trim(), classroom: data.classroom, round: data.round, answers: answers.map(a => ({ result: a.result, explanation: a.explanation, image: a.image, imageName: a.imageName, ...(a.imagePath ? { imagePath: a.imagePath, imageBucket: IMAGE_BUCKET } : {}), ...(a.rows ? { rows: a.rows } : {}) })), submitted_at: submittedAt }),
