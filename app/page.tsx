...
type Work = { version: 4; name: string; classroom: string; answers: Answer[]; submittedAt: string; scores: (number | null)[]; feedback: string[]; reviewed: boolean };
...
export default function Home() {
 ...
 const task = tasks[Math.min(active, 4)];
 const answer = work.answers[Math.min(active, 4)];
 ...
 return <div className="app">
  ...
  <div className="shell"><aside className="sidebar"><h2>데이터 분석 미션</h2><div className="side-divider"/><div className="nav-label"><span>30점</span></div><nav aria-label="수행 단계">{tasks.map((t, i) => <button key={t.short} className={`nav-item ${active === i ? 'active' : ''}`} onClick={() => navigate(i)}><span className="nav-num">{complete[i] ? '✓' : `0${i + 1}`}</span><span>{t.short}</span><span className="nav-arrow">›</span></button>)}<button className={`nav-item review-nav ${active === 5 ? 'active' : ''}`} onClick={() => navigate(5)}><span className="nav-num">▤</span><span>{teacher ? '전체 답안 평가' : '검토 및 제출'}</span><span className="nav-arrow">›</span></button></nav><div className="progress-card"><div><span>포트폴리오 작성</span><b>{count}<small> / 5</small></b></div><div className="progress-track"><i style={{ width: `${count * 20}%` }}/></div><p>각 수행의 필수 항목을 작성하면 완료돼요.</p></div></aside>
  <main>{active === 5 && <><div className="breadcrumb">데이터 탐구 포트폴리오 <span>/</span> {teacher ? '교사 평가실' : '학생 작업실'}<span className="save-state">● {saved}</span></div>
   <section className="workspace-heading"><div><p className="eyebrow">{teacher ? 'TEACHER REVIEW' : active === 5 ? 'YOUR PORTFOLIO' : `PERFORMANCE 0${active + 1}`} <span>{active < 5 ? '6점' : '총 30점'}</span></p><h1>{active === 5 ? teacher ? '증거와 생각을 함께 평가해요' : '나의 탐구를 완성해요' : task.title}</h1><p>{active === 5 ? '다섯 가지 분석 결과와 설명을 확인하고 포트폴리오를 마무리하세요.' : task.subtitle}</p></div><button className="button secondary import-button" onClick={() => importInput.current?.click()}>↥ 답안 파일 불러오기</button></section></>}
   ...
   {active < 5 ? <><div className="task-layout"><div className="task-main"><section className="guide-card"><div className="section-heading"><span className="section-icon">↗</span><div><h2>{task.short}</h2><p>{task.tag}</p></div><span className="badge">작업 안내</span></div><p className="instruction-summary">{task.steps.join(" ▶ ")}</p></section>
   ...
   <div className="portfolio-summary"><div><span>작성 완료</span><b>{count} <small>/ 5개 수행</small></b></div><div><span>제출 상태</span><b>{locked ? '파일 생성 완료' : '작성 중'}</b></div><div><span>교사 평가</span><b>{work.reviewed ? `${Number((work.scores.reduce<number>((a, b) => a + (b ?? 0), 0) * 0.24).toFixed(2))} / 30점` : '평가 전'}</b></div></div>
